<table class="table table-striped table-bordered" style="font-size: 12px;">
    ...
    <tbody>
        ...
        @php
        $jsonString = json_encode($slide);
        $decodeImage = json_decode($jsonString, true);
        @endphp
        <tr>
            <td>
            ...
            <td>
                @foreach($decodeImage as $item => $val)
            ...
            <td class="text-center js-switch-{{ $slide->id }}">
                <input type="checkbox" value="{{ $slide->publish }}" class="js-switch status " data-field="publish"
                    data-model="{{ $config['model'] }}" {{ ($slide->publish == 2) ? 'checked' : '' }} data-modelId="{{
                $slide->id }}" />
            </td>
