(function ($) {
    ...
    HT.menuRowHtml = (option) => {
        let html
        let $row = $('<div></div>').addClass('row mb10 menu-item ' + ((typeof (option) != 'undefined') ? option.canonical : '') + '')
        const columns = [
        ...
        columns.forEach(col => {
            let $col = $('<div></div>').addClass(col.class)
        ...
        let $removeCol = $('<div></div>').addClass('col-lg-2 text-center')
        let $removeRow = $('<div></div>').addClass('form-row text-center')
        let $a = $('<a></a>').addClass('delete-menu')
        let $i = $('<i></i>').addClass('fa fa-minus')
    ...
    HT.getByModel = () => {
        $(document).on('click', '.menu-module', function () {
            let _this = $(this)
            let option = { model: _this.attr('data-model') }
    ...
    HT.menuLinks = (links) => {
        let nav = $('<nav></nav>')
        if (links.length > 3) {
            let paginationUl = $('<ul></ul>').addClass('pagination')
            $.each(links, function (index, link) {
                ...
                let li = $('<li></li>').addClass('liClass')
                if (link.label == 'pagination.previous') {
                    let span = $('<span></span>').addClass('page-link').attr('aria-hidden', true).html('<')
                    li.append(span)
                }
                    ...
                    let a = $('<a></a>').addClass('page-link').text(link.label).attr('href', link.url).attr('data-page', link.label)
                    li.append(a)
                }
